...
3. 如果用户要求生成代码（HTML页面、原型图、网页、程序等），必须直接输出完整可运行的代码。参考来源仅作背景参考，不要把参考来源列表当作最终回答。
4. 你需要自行判断输出深度：
...
7. 如果有个别工具失败，只在确实影响结论时用一句话简短说明，不要展开报错细节。
8. 始终保留来源感，尽量给出链接，但不要把原始结果整段照搬。
9. 如果是目录/文件搜索任务，直接给出找到的路径、判断依据和下一步建议；不要只说"你可以自己去看"。
10. 如果用户要求像公众号文章、简报、日报、周报一样写，请直接产出可交付正文，而不是素材列表。
11. 使用简洁、自然、专业的中文 Markdown，不要解释你选择了哪种模式。`
...
4. 如果用户要求生成代码（HTML页面、原型图、网页、程序等），必须直接输出完整可运行的代码，不要只罗列参考资料或来源。
5. 如果是搜索、目录、资讯、热榜、文章整理类任务，必须形成对用户可交付的最终结果，不要只汇报中间步骤。
6. 必须将工具返回的搜索结果与用户原始问题紧密结合，基于搜索到的资料来回答或完成任务，而不是简单罗列搜索结果。例如用户要求写文档，应将搜索资料融入文档内容中；用户要求查询信息，应基于搜索结果整理出完整答案。
7. ${lockedMcpServerId ? `本轮后续若继续调用 MCP，只允许使用 ${await getMcpServerLabel(lockedMcpServerId) || lockedMcpServerId} 这个 MCP 服务器下的工具，不要切换到其他 MCP。` : '如果本轮已经开始使用某个 MCP 服务器，后续优先留在同一命名空间内完成。'}`
      }
    ]
...
function looksLikeIncompleteToolIntent(text: string): boolean {
  ...
  // 检测裸 JSON 工具调用（本地模型可能输出了但格式不对未被解析）
  if (/\{\s*"tool"\s*:\s*"[^"]+"/.test(text)) return false
  // 检测中间态意图
  const intentPatterns = [
    /((让我|我来|现在|接下来|下面).*(搜索|查找|查看|读取|分析|整理|生成|执行|调用|使用工具))/,
    /((正在|立刻|马上).*(搜索|查找|查看|执行|调用))/,
    /((我会|将会|需要|应该).*(搜索|查找|分析|整理|调用|使用))/,
    /((我需要|需要).*(使用|调用).*(工具|功能))/,
    /((搜索|查找|查询|检索).*(一下|看看|相关信息|相关内容))/,
  ]
...
请现在做出决定：要么输出 [TOOL_CALL] 标签调用工具，要么直接给出最终回答。`
